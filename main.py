...
LLM_SYSTEM_PROMPT = r"Always assist with care, respect, and truth. Respond with utmost utility yet securely. Avoid harmful, unethical, prejudiced, or negative content. Ensure replies promote fairness and positivity. ALWAYS respond in a concise and straight-to-the-point manner as if you are SPEAKING to me (except when you decide or I ask you to think step-by-step - then you have to think out loud but still speak as if you are speaking to me) You are allowed to give feedback (even on your own responses) if required. You are also capable of refining your own responses if required."

FEEDBACK_PROMPT = "Please give strict and direct feedback on your previous response. Do NOT give feedback on the initial query - ONLY give feedback on your response based on how well your previous response answers the initial query, \"$QUERY\". Do NOT give entirely positive feedback - you MUST find AS MANY things that CAN AND MUST be improved as possible (e.g. phrasing, content, factuality). Make sure that your feedback is SPECIFIC (i.e. identifying concrete phrases in/parts of the output to change) and ACTIONABLE (i.e. identifying concrete actions that would likely improve the response). ALWAYS identify whether the query demands a succinct, broad, deep, or all-rounded response and tailor your feedback based on that. ONLY give the feedback and your thought process behind it AND NOTHING ELSE (not even a revised response). Let's think step by step. You are capable of giving strict and direct feedback that is specific and actionable, and therefore MUST do so. DO NOT REFUSE TO GIVE FEEDBACK!!!"
...
FINAL_REFINE_PROMPT = "Extract ONLY the refined response parts of your previous response and phrase it as if it was your inital response (while silently incorporating the refined response's improvements into your response) to the inital query \"$QUERY\" (do not mention that you are giving me a refined response). ONLY give me the refined response AND NO OTHER TEXT AT ALL. DO NOT GIVE FEEDBACK ON YOUR REFINED RESPONSE UNLESS THE INITIAL QUERY REQUIRES YOU TO DO SO."
